...
export function kiesBericht(opts: {
  ...
  // Alleen een echte naam gebruiken. "Jij" of leeg telt niet als naam.
  const naam = spelerNaam?.trim();
  const heeftNaam = !!naam && naam.toLowerCase() !== "jij";

  if (fase === "einde") {
    if (winnaarIsMens)
      return heeftNaam
        ? `Masha bun, ${naam}! Je hebt gewonnen. Netjes gespeeld.`
        : `Masha bun! Je hebt gewonnen. Netjes gespeeld.`;
    return heeftNaam
      ? `${winnaarNaam ?? "De ander"} was net sneller. Volgende keer pak jij 'm, ${naam}.`
      : `${winnaarNaam ?? "De ander"} was net sneller. Volgende keer pak jij 'm.`;
  }

  if (fase === "wachten" && ronde === 0) {
    return heeftNaam
      ? `Hi ${naam}, leuk dat je er bent! Ik ben de ${SPELLEIDER_NAAM}. Draai de eerste kaart maar om als je klaar bent.`
      : `Hi, leuk dat je er bent! Ik ben de ${SPELLEIDER_NAAM}. Draai de eerste kaart maar om als je klaar bent.`;
  }
